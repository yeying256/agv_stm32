...
//	uint8_t* tmp;
	...
	uint8_t index=0;
	uint8_t len_tmp;
	Append_CRC8_Check_Sum( (uint8_t *)(&Header),4);
	len_tmp=sizeof(Header);
	memcpy(buf,&Header,len_tmp);
	
	
	index+=len_tmp;				//偏移到cmd起始位置
	
	len_tmp=sizeof(CMD);
	memcpy(buf+index,&CMD,len_tmp);
	
	index+=len_tmp;				//偏移到data起始位置
	
	memcpy(buf+index,data,len);
	
	index+=len;//偏移到CRC16的起始位置
	
	Append_CRC16_Check_Sum(buf,index+2);//这里的长度是整个数据的长度，而不只是data的长度
	
	index+=2;//这里的index也是长度
	tmp_fuck=index;
	
	HAL_UART_Transmit(&huart5,buf,index,5);
	
//	HAL_GPIO_WritePin(GPIOH,GPIO_PIN_8,GPIO_PIN_SET);//开启485使能
//	
//		HAL_UART_Transmit(&huart3,buf,index,3);
//		
//	
//	HAL_GPIO_WritePin(GPIOH,GPIO_PIN_8,GPIO_PIN_RESET);//关闭485使能
//	HAL_UART_Transmit(&huart3,data,strlen((char *)data),4);
